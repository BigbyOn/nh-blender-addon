...
class NH_OT_paste_common_material(bpy.types.Operator):
    """Paste a common material path"""
    
    bl_label = "Paste Common Material"
    bl_idname = "nh.paste_common_material"
    ...
    def draw(self, context):
        ...
        layout = self.layout
        layout.template_list("NH_UL_common_data_materials", "A3OB_common_materials", scene_props, "items", scene_props, "items_index", item_dyntip_propname="value")
...
class NH_OT_paste_common_procedural(bpy.types.Operator):
    """Paste a common procedural texture"""
    
    bl_label = "Paste Common Procedural"
    bl_idname = "nh.paste_common_procedural"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def draw(self, context):
        scene_props = context.scene.a3ob_commons
        layout = self.layout
        layout.template_list("NH_UL_common_data_procedurals", "A3OB_common_procedurals", scene_props, "items", scene_props, "items_index", item_dyntip_propname="value")
...
class NH_UL_common_procedurals(bpy.types.UIList):
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname):
        layout.label(text=item.name)


class NH_PT_material(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if texture_type == 'TEX':
            ...
            row_raw.operator("nh.paste_common_procedural", text="", icon='PASTEDOWN')
            row_raw.prop(material_props, "color_raw", text="", icon='TEXT')
        
        row_material = layout.row(align=True)
        row_material.operator("nh.paste_common_material", text="", icon='PASTEDOWN')
        row_material.prop(material_props, "material_path", text="", icon='MATERIAL')


classes = (
    NH_OT_paste_common_material,
    NH_OT_paste_common_procedural,
    NH_UL_common_procedurals,
    NH_PT_material,
)
