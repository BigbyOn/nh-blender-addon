...
for %%A in (%*) do (
    ...
    if /I "%%~A"=="--allow-single-file" set "ALLOW_SINGLE_FILE=1"
...
if exist "%ADDON_PACKAGE_DIR%\__init__.py" (
    ...
    if not "%ALLOW_SINGLE_FILE%"=="1" (
        echo ERROR: package addon folder was not found:
        echo   %CD%\%ADDON_PACKAGE_DIR%\__init__.py
        echo.
        echo Single-file %ADDON_SINGLE_FILE% exists, but package mode is required by default
        echo because tools\xray_tex_converter\dds_python.py must be included in the ZIP.
        echo.
        echo Use --allow-single-file only for legacy builds without bundled tools.
        exit /b 1
    )
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    ...
    "$packageEntry=Join-Path $repo ('%ADDON_PACKAGE_DIR%' + '\__init__.py');" ^
    "if(Test-Path -LiteralPath $single){ $sourceFiles += $single };" ^
    "if(Test-Path -LiteralPath $packageEntry){ $sourceFiles += $packageEntry };" ^
    "if($sourceFiles.Count -eq 0){ throw 'No addon source files found for version update'; }" ^
    ...
    "  $v=Get-BlInfoVersion $source;" ^
    "  if($sourceMax -eq $null -or (Version-Greater $v $sourceMax)){ $sourceMax=$v }" ^
    "}" ^
    "$latestZip=$null;" ^
    ...
    "foreach($source in $sourceFiles){ Set-BlInfoVersion $source $releaseVersion }" ^
    "$enc=New-Object System.Text.UTF8Encoding($false);" ^
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    ...
    "    $src=Join-Path $repo $packageDir;" ^
    "    $dst=Join-Path $tempRoot $packageDir;" ^
    ...
    "        ($packageDir + '/tools/xray_tex_converter/dds_python.py')," ^
    "        ($packageDir + '/tools/xray_tex_converter/converter.js')" ^
    "      );" ^
    "      foreach($req in $required){" ^
    "        if($entries -notcontains $req){ throw ('Archive validation failed. Missing entry: ' + $req); }" ^
    "      }" ^
    "    } else {" ^
    "      if($entries -notcontains $singleFile){ throw ('Archive validation failed. Missing entry: ' + $singleFile); }" ^
    "    };" ^
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    ...
    "      ('## ' + $v + ' - ' + $date)," ^
    "      ''," ^
    "      '- Package addon ZIP build with bundled texture converter tools.'," ^
    "      '- Built-in Python DDS converter is included in NH_Blender/tools/xray_tex_converter/dds_python.py.'," ^
    "      '- ZIP validation now checks package structure and forward-slash archive paths.'," ^
    "      ''" ^
...
echo   build_addon_zip.bat --allow-single-file
echo.
...
echo --allow-single-file allows legacy NH_Blender.py ZIP without bundled tools.
exit /b 0
