...
### Исправлено
- Fake Terrain: настоящие замкнутые выпуклые компоненты, измеряемые допуски над/под визуалом, контроль границ и лимит треугольников на компонент. Корректная триангуляция вогнутых полигонов, сохранение отверстий и раздельных уровней.
- Cylinder/Pipe: определение оси по вершинам и кольцам рёбер вместо bounding box; ручной выбор оси, настройки боксов через F9, поддержка смещённых углов внутренних/внешних колец.
- Create Pipe создаёт отдельные выпуклые секции. Толщина стенок учитывает эллиптическое сечение; сварка сохраняет независимость компонентов.
- Точный Re-Convex без выборки экстремумов; явный режим Exact при нулевом бюджете; ограничение геометрической ошибки Simplify.
- Откат Rebuild и многокомпонентной генерации при ошибке. Исправлены индексы при сварке, полюса Sphere/Capsule и дублирование экватора Capsule; Capsule получает реальные точки при Follow Source Angle.
- OBB сравнивает направления с учётом мирового масштаба. Точки и прокси на внутренних границах Model Split получают одного владельца.
- Collision QA проверяет нулевые площади/объёмы, выпуклость, дубли и пересечения несмежных граней. Geometry Audit использует BVH для лучевых запросов; очистка отделяет shared mesh от невыбранных копий.
- Исправлен запуск unittest discovery. Добавлены регрессионные и интеграционные проверки Blender, а также независимая проверка высоты на пользовательском terrain.

## [0.6.2.22] - 2026-09-13

### Добавлено
- В `P3D Tools & Checks` добавлен `Snap Magnet Check` (`Check & Assemble Visual LODs`): автоматический поиск и сборка цепочек snap points по всем Memory LOD сцены — непрерывность `01 -> 02 -> 03...`, пары `_0/_1`, дубликаты, неполные пары, пропуски ID, неверные расстояния и разрывы цепи. Посторонние P3D без `.sp_` игнорируются, односторонние коннекторы к отсутствующим моделям выводятся предупреждениями. Для старого локального нумератора `01, 02, 01, 02` порядок восстанавливается по P3D roots `_p01_, _p02_...`. Перемещаются только visual Resolution LOD.
- Snap Points: pair ID выбирается автоматически по существующим `.sp_` vertex groups; ось `X/Y/Z` задаёт порядок точек `0/1` и попадает в имя только при включённом `Include Axis in Name`.
- В экспериментальном collider добавлен оператор `Hull Selected Vertices` — выпуклая оболочка по выделенным вершинам в Edit Mode.
- Добавлен оператор `Restore Material Preview`: пересобирает превью-ноды текстур выбранной P3D-модели и переключает текущий 3D-вид в Material Preview.

### Изменено
- Внутренний P3D backend (`NH_bundle`) используется первым во всех NH import/export workflow; внешний Arma 3 Object Builder остаётся резервом совместимости.
- Drag & Drop `.p3d` сразу добавляет файлы в очередь `Import/Export Planner` и немедленно обновляет список.
- Раздел `Fixes` переименован в `P3D Tools & Checks`.
- `Plain Axis`: обычное удаление возвращает все LOD в исходные transforms, удаление с `Save Z` сохраняет высоту `Resolution 0` и выравнивает по ней `Memory` и остальные LOD.
- Batch Export сохраняет мировые матрицы и исходную видимость объектов и коллекций.

### Исправлено
- Импорт и Batch Export распознают старые P3D с большим накопленным сдвигом `Resolution`/`Geometry`/`Roadway` LOD относительно `Resolution 0` и возвращают их в одну систему координат; `Memory` не перемещается. В отчёте выводится число выровненных LOD (`realigned N displaced LOD(s)`).
- Поиск текстур при импорте дополнительно опирается на папку исходного `.p3d`: project-relative пути ищутся от каталога модели до корня диска, а дублирование имени папки (`P:\Objects\...`) отбрасывается.
- Snap Points: создание останавливается до изменения Memory LOD, если выделено не ровно две вершины или рядом с обеими позициями нет вершин одновременно в A Target и V Target (допуск `Target Match Distance`); выбранные A/V targets не переориентируются автоматически.
- Восстановлена кириллица в названиях и описаниях свойств и операторов вместо повреждённого текста.

## [0.6.2.8] - 2026-09-06

### Добавлено
- Добавлен `Geometry Audit`: сканирование выбранных Geometry LOD или активной P3D-модели — сырые и эффективные компоненты, вершины и острова без граней, мелкие компоненты, подозрительные и явные вложенные оболочки; результаты по каждому LOD, выбор проблемных вершин (`Select Faceless/Tiny/Nested`) и `Clean Safe Garbage` с self-test набором.
- Импорт Terrain Builder TXT в панели Import/Export planner и меню File → Import.
- Автоматический поиск исходных MLOD P3D, преобразование yaw/pitch/roll и масштаба,
  поправка центра габаритов всех LOD по XY для точной сборки секций тоннелей.
- Выбор начала координат, связанные/независимые копии и сохранение последней папки моделей.
- Поддержка встроенного NH P3D backend и активного Arma 3 Object Builder без отдельного TXT-аддона.
- Проверка данных до расстановки и откат созданных объектов при ошибке без изменения очереди планировщика.
- Тесты парсера и импорта восьми секций Yantar, включая повторный импорт и откат при сбое.
- Ссылки аддона и адрес публикации в сборочном скрипте направлены на форк T3Z-ONE.

## [0.5.4.8] - 2026-09-03

### Исправлено
- `Add New P3Ds + Icons` больше не зависит от непостоянного флага `Textured rendered previews`: режим иконок наследуется от существующего кеша библиотеки (или из флага, если кеша еще нет). Новые `.p3d` добавляются с текстурными иконками, а `.paa` при недостающем PNG-кеше конвертируется на лету.
- Починено сравнение кеша иконок: сканирование инкрементальных объектов шло по `_NH_incremental` в корне кеша, хотя папки лежат в кешах исходных подпапок (`Environment\Water\_NH_incremental\...`) — теперь геометрийные иконки, требующие обновления, снова находятся и пересоздаются.
- Иконка библиотека ассета теперь содержит ровно один Resolution LOD (`0` — самый детальный, как `first_lod_only`), а не все LOD файла: лишние Geometry / View Geometry / Fire Geometry / Roadway / прочие Resolution LOD удаляются при построении кеша.
- `Add Custom Asset` так же наследует текстурный/геометрийный режим существующего кеша `Custom`.

## [0.5.4.7] - 2026-07-26

### Добавлено
- В `Cache Manager` под `Add New P3Ds + Icons` добавлена команда `Force Rebuild All Icons + Textures`: она принудительно обновляет PNG-кеш используемых `.paa`, очищает кеш библиотек `Common` / `Environment` и заново рендерит все иконки с материалами.
- Asset preview поддерживает точку-селекшен `nh_cam` в Memory LOD. Суффикс угла (`nh_cam_90`, `nh_cam_180`, `nh_cam_270`) поворачивает ракурс вокруг вертикальной оси.

### Исправлено
- Флаг текстурных иконок снова вызывает rendered preview вместо всегда включавшегося geometry wireframe.
- Масштаб ортографической камеры и geometry-preview рассчитывается по выбранному ракурсу, чтобы модель максимально заполняла иконку без обрезания.
- Последующие `Add New P3Ds + Icons` сохраняют текстурный режим после полной текстурной пересборки.

## [0.5.3.16] - 2026-06-19

### Исправлено
- Texture export/cache больше не использует `P:\NH_ObjectTextures` как дефолтный output/cache root; новые значения уходят в `%LOCALAPPDATA%\NH_Blender\NH_ObjectTextures`.
- Persisted UI state поднят до версии `4`: старые сохраненные дефолты `P:\NH_ObjectTextures` / `P:\NH_ObjectTextures\` игнорируются при восстановлении, чтобы Mikero не подхватывал созданную аддоном top-level P-drive папку в `P:\temp`.

## [0.5.3.15] - 2026-06-19

### Исправлено
- `Line Grid Split` удаляет временные cell-cutter объекты вместе с их mesh-data, чтобы после нарезки не оставались orphan datablocks.
- Финально синхронизирован пакетный `NH_Blender/__init__.py` с корневым `NH_Blender.py` перед ZIP-сборкой.

## [0.5.3.14] - 2026-06-19

### Изменено
- `Grid Cutter Split` заменен на `Line Grid Split`: сетка задается как `Parts X/Y`, например `3x3`, и одной кнопкой создает части `имя_модели_p01.p3d`, `имя_модели_p02.p3d` и далее.
- Добавлены editable cut-lines: `Create/Edit Cut Lines` создает линии разреза, которые можно сдвинуть вручную перед `Split To _p Parts`.
- Убран видимый workflow с cutter-cube коллекцией, размерами ячеек и `Count Z`; разрез идет по X/Y-линиям исходного bounds или по вручную сдвинутым cut-lines.

### Исправлено
- Пакетный `NH_Blender/__init__.py` снова синхронизирован с корневым `NH_Blender.py`, чтобы новая панель и операторы попадали в ZIP-сборку.

## [0.5.3.13] - 2026-06-19

### Исправлено
- Синхронизирован пакетный `NH_Blender/__init__.py` с корневым `NH_Blender.py`, чтобы `Grid Cutter Split` был виден в ZIP-сборке и локальном package deploy.
- В `Menu Settings` пункт `Object Builder / Clutter` переименован в `Clutter Proxies (DayZ)`, чтобы панель можно было явно найти и скрыть.

### Изменено
- Версия аддона поднята до `0.5.3.13`.
