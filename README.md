...
`Create Cylinder` создаёт один выпуклый цилиндр; `Cylinder Boxes` создаёт приближение из боксов по исходной геометрии.

Для пайпа в **0.6.2.25** используется последовательность:

1. Выделите исходное колесо/кольцо или нужную часть меша и нажмите `Create Pipe`. Создаётся отдельный редактируемый пайп в коллекции `NH Pipe Guides`, вне LOD. Он становится активным.
2. В `Pipe Editor` задайте **Outer Diameter**, **Inner Diameter**, **Depth**, **Box Segments** и при необходимости **Segment Rotation**. Это диаметры, а не множители радиуса. Изменения сразу обновляют тот же объект. Редактор остаётся доступен после сохранения и открытия `.blend`; также доступны параметры F9.
3. Нажмите **Convert Pipe to Boxes** или **Pipe to Boxes**. Редактируемый пайп заменяется отдельными замкнутыми выпуклыми боксами в выбранном Geometry LOD. У каждого бокса 8 вершин и 6 четырёхугольных граней (12 треугольников). Прежняя геометрия LOD сохраняется. Для изменения диаметра после преобразования отмените преобразование через Undo.

Повторное `Create Pipe` при активном редактируемом пайпе изменяет его, не создавая дубликат. Если внутренний диаметр больше внешнего, редактор сохраняет последнюю допустимую форму и показывает ошибку. При сбое преобразования остаются и пайп, и предыдущая геометрия LOD. Несколько выбранных редактируемых пайпов можно преобразовать одновременно.

Ось определяется по реальным вершинам и кольцам рёбер. Проверяются группы коротких и длинных рёбер, поскольку у тонкого колеса соотношение длин обратное длинной трубе. До создания можно выбрать `Round Axis`: Auto, Scene Vertical или Local X/Y/Z. Готовый пайп можно перемещать, вращать и масштабировать обычными инструментами Blender; преобразование учитывает его трансформацию. Поля диаметров задают размеры до дополнительного Object Scale.

Созданные старыми версиями компоненты в LOD не становятся редакторами автоматически. Новый рабочий процесс начинается с `Create Pipe` на исходном визуале; существующие коллизии не удаляются без выбора пользователя.

### Fake Terrain в 0.6.2.24

Выделите поверхности визуала в Edit Mode и нажмите Generate Fake Terrain. Генератор собирает отдельные замкнутые 3D convex hull, объединяя соседние участки только при соблюдении точности и лимита сложности. Отверстия и независимые уровни не заполняются общей оболочкой. Вертикальные стены не являются поверхностью Fake Terrain и пропускаются.

- `Above Visual` и `Below Visual` — независимые допуски вверх и вниз, по умолчанию по 0.10 м. Можно установить собственные значения.
- `Patch Size` — максимальный размер области объединения; `Triangles per Hull` — лимит треугольников одного компонента, по умолчанию 32.
- `Thickness` задаёт глубину нижней части оболочки. На границах используется половина меньшего допуска, чтобы соседние компоненты не образовывали ступеньку в сумму двух допусков.
- Прежний `Min Patch Size` больше не ограничивает точность и скрыт из панели. Старое поле оставлено для совместимости скриптов.

Отклонения проверяются по пересечениям треугольников готовой верхней поверхности с исходным визуалом. Нулевой допуск и сложный рельеф могут потребовать много компонентов; алгоритм не гарантирует минимально возможное их число. Повторное нажатие добавляет новые компоненты в выбранный LOD.

`Re-Convex` использует все вершины выбранных компонентов. Для Generate/Simplify Hull значение `Max Hull Triangles = 0` означает точную оболочку. При положительном лимите `Max Shape Error` ограничивает допустимую потерю формы в метрах; несовместимые ограничения приводят к отказу с сохранением предыдущей геометрии.
...
# Ladder Points (Memory LOD)

Раздел расположен под Snap Points и создаёт Memory-точки и бокс во View Geometry для лестницы DayZ.

1. Выберите **Standard** или **Middle Exit**.
2. Укажите **Bottom Step / Top Step**, при необходимости **Middle Step**. Можно выбрать два отдельных меша и нажать **Use Two Selected Steps** или выделить две несвязанные части в Edit Mode. Кнопка рядом с каждым полем захватывает конкретную выделенную ступеньку.
3. Выберите **Top Exit: Front / Left / Right / Both**. Для промежуточного выхода доступны **Left / Right / Both**. **Approach Side / Flip Side** задают сторону подхода.
4. Задайте **Ladder ID** или оставьте `0` для первого свободного номера. **Create / Update Ladder** создаёт/обновляет этот ID; **+** позволяет добавить следующую лестницу.

Memory и View Geometry создаются автоматически в модели ступеньки, либо выбираются в соответствующих полях. Во View Geometry добавляется отдельный замкнутый бокс (8 вершин, 6 граней) с группами `ladderN` и `ComponentXX`. Повторное создание с тем же ID заменяет его точки и бокс, сохраняя остальные лестницы. Изменения обоих LOD откатываются при ошибке.

В **Offsets** настраиваются высоты входов и взаимодействия, подъём бокового выхода, глубина бокса и запас. Центр выбранной ступеньки задаёт уровень входа/выхода; если он отличается от уровня пола, используйте поправку высоты. По двум симметричным ступеням невозможно определить лицевую сторону однозначно — проверьте **Flip Side**. После изменения топологии захваченных частей меша повторите захват.

Структура соответствует пользовательскому laddernest и [документации Bohemia](https://community.bistudio.com/wiki/DayZ%3ALadders_on_buildings). Geometry/Roadway, свободное место для персонажа и конфигурация здания проверяются отдельно; готовую лестницу следует проверить в DayZ.
