...
`Create Cylinder` создаёт один выпуклый цилиндр. `Create Pipe` создаёт кольцо из раздельных замкнутых выпуклых секций, сохраняя отверстие. `Cylinder Boxes` и `Pipe Boxes` создают приближение из боксов непосредственно по выбранной исходной геометрии. Эти команды являются альтернативами: для создания боксов не нужно сначала добавлять Cylinder/Pipe в тот же LOD.

Ось определяется по реальным вершинам и кольцам рёбер. Проверяются группы коротких и длинных рёбер, поскольку у тонкого колеса соотношение длин обратное длинной трубе. Для неоднозначной формы есть `Round Axis`: Auto, Scene Vertical и Local X/Y/Z. Настройки сегментов, оси и размеров доступны через F9. `Inner Radius Factor` — доля внешнего радиуса; `Outer Radius Scale` — множитель. `Pipe Thickness` ограничивает минимальную толщину стенки, в том числе на эллипсе; слишком большая толщина вызывает понятный отказ.
